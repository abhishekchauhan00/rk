...
{
	if($_POST["txtname"]=="" || $_POST["txtage"]=="" || $_POST["txtclass"]=="" || $_POST["txtphone"]=="" || $_POST["txtemail"]=="" ||  $_POST["txtpschool"]=="" || $_POST["txtmessage"]=="" )/*$_POST["txtemail"]=="" ||*/
	{
		$valid=false;
	}
	...
	if($valid==false)
	{
			header("location:../enquiry.php?err=one of the requried field is blank!");
		return;
	}
        ...
        function smtpmailer($to, $from, $from_name = 'rkinternational.org', $subject, $body, $is_gmail = false)
    {
       ...
       if($is_gmail)
        ...
        if(!$mail->Send())
        {
			...
			header("location:../enquiry.php?err=$error");
            return true;
        }
        else
        {
